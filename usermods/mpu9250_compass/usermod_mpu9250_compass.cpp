...
// ICM-20948 register map (USER BANK 0) - same family as the MPU-6500 but the
// WHO_AM_I lives at 0x00 (0xEA) and several addresses differ.
constexpr uint8_t ICM_WHO_AM_I      = 0x00; // 0xEA
...
constexpr uint8_t AK16_CNTL2 = 0x31; // MODE[4:0] bits 3:0, 0x08 = continuous 100 Hz
constexpr uint8_t AK16_CNTL3 = 0x32; // bit0 = SRST (soft reset)
...
class Mpu9250Driver {
  ...
  private:
    uint8_t _addr = 0x68;
    bool    _isICM = false;   // ICM-20948 (banked register map) instead of MPU-6500
    float   _magSens[3] = {1.0f, 1.0f, 1.0f};
    uint8_t _whoAmI = 0;      // MPU-6500 / ICM-20948 WHO_AM_I as read from the chip
    uint8_t _magWhoAmI = 0;   // magnetometer WIA as read from the chip (AK8963/AK09916)
    MagPath _magPath = MAG_NONE;

    // ---- magnetometer via I2C bypass: it is directly on the bus at 0x0C ----
    bool initMagBypass() {
      using namespace mpu9250;
      uint8_t wia = 0;
      _magWhoAmI = 0;
      if (_isICM) {
        // AK09916 (embedded in the ICM-20948): WIA1 = 0x48, WIA2 = 0x09
        if (!readReg(AK16_I2C_ADDR, AK16_WIA1, wia) || wia != 0x48) return false;
        _magWhoAmI = wia;
        writeReg(AK16_I2C_ADDR, AK16_CNTL3, 0x01); // soft reset
        delay(10);
        writeReg(AK16_I2C_ADDR, AK16_CNTL2, 0x08); // continuous, 100 Hz
        delay(20);
        _magPath = MAG_BYPASS;
        return true;
      }
  ...
  public:
    ...
    bool isICM() const        { return _isICM; }
    ...
    bool readAccelGyro(int16_t *acc, int16_t *gyr) {
      using namespace mpu9250;
      if (_isICM) {
        // ICM-20948: ACCEL_XOUT_H(0x2D)..GYRO_ZOUT_L(0x38), 12 bytes, no temp gap
        uint8_t buf[12];
        if (!readRegs(_addr, ICM_ACCEL_XOUT_H, buf, 12)) return false;
 ...
 *     Speed sets the base drip pause; Flow (Custom 1) shortens it so several
 *     grains are in flight at once, making the sand flow faster. Level the
 *     device to stop, reverse the tilt to drain it back.
 ...
 *   speed     = base pause between grains (1D) / pour rate (2D)
 *   intensity = sand amount
 *   custom1   = Flow: shortens the drip pause (1D) / sim steps (2D)
 */
struct FallingSandData {
  ...
  uint8_t  dripsCount = 0; // in-flight grains (positions stored in the tail array)
...
static void mode_falling_sand() {
  ...
  if (cells == 0) return;
  // 1D needs an int16 array for in-flight grain positions; 2D needs a byte grid
  ...
  if (d->w != W || d->h != H) {
    ...
    d->w = W; d->h = H;
    d->sandCount = 0;
    d->N = 0; d->pile0 = 0; d->pileW = 0; d->dripsCount = 0; d->dir = 0;
  ...
  if (H == 1) {
    ...
    int16_t *drips = reinterpret_cast<int16_t*>(tail); // in-flight grain positions

    // (re)initialise the pile when the sand amount changed
    if (d->N != N) {
      d->N = N;
      d->pile0 = N; d->pileW = 0;
      d->dripsCount = 0; d->paused = 0;
    }
    ...
    // release interval: speed sets the base pause (default ~50% faster),
    // Flow (Custom 1) shortens it further so grains overlap
    uint32_t interval = 30 + (255 - SEGMENT.speed) * 5 / 2;   // ~30ms..668ms
    interval = interval * (256 - SEGMENT.custom1) / 256 + 10; // Flow shortens the pause
    ...
    // move every in-flight grain one cell per ~60 ms; land them at the low pile
    if (d->dripsCount > 0 && (now - d->lastFallMove) >= 60) {
      d->lastFallMove = now;
      for (uint8_t k = 0; k < d->dripsCount; ) {
        drips[k] += d->dir;
        int16_t limit = (d->dir > 0) ? (int16_t)(W - 1 - d->pileW) : (int16_t)d->pile0;
        if ((d->dir > 0 && drips[k] >= limit) || (d->dir < 0 && drips[k] <= limit)) {
          if (d->dir > 0) d->pileW++; else d->pile0++;
          for (uint8_t m = k; m + 1 < d->dripsCount; m++) drips[m] = drips[m+1];
          d->dripsCount--;
        } else {
          k++;
        }
    ...
    // render: source pile + low pile + all in-flight grains
    SEGMENT.fill(SEGCOLOR(1));
    for (uint16_t i = 0; i < W; i++) {
      bool lit = false;
      if (i < d->pile0) lit = true;                 // low-index pile
      if (i >= W - d->pileW) lit = true;            // high-index pile
      for (uint8_t k = 0; k < d->dripsCount; k++) if (drips[k] == (int16_t)i) { lit = true; break; }
...
void Mpu9250Compass::initSensor() {
  ...
  if (sensorEnabled) {
    if (sensorType != 2 && _mpuDriver.begin(i2cAddress, sdaPin, sclPin)) {
      ...
      _mpuIsICM  = _mpuDriver.isICM(); // ICM-20948 (0xEA) vs MPU-6500/9250
      _accelOK = true;
  ...
  magType   = (_sensorKind == 2) ? (uint8_t)_gy271Driver.magType()
                                 : (_magOK ? (_mpuIsICM ? 5 : 1) : 0);
  // magType: 0=none, 1=AK8963 (MPU-9250), 5=AK09916 (ICM-20948)
  scanI2CBus();
  readErrors = 0;
...
const char* Mpu9250Compass::statusText() const {
  ...
  if (_accelOK)
    return "MPU/ICM accelerometer OK - no magnetometer (compass unavailable, tilt effects work)";
  if (whoAmI == 0)
    return "no MPU-9250/ICM-20948 and no GY-271 magnetometer found - check wiring, pins and power";
  return "magnetometer not found (checked I2C bypass) - this module appears to be an MPU-6500/ICM without a compass";
}
...
void Mpu9250Compass::addToJsonState(JsonObject& obj) {
  ...
  top["whoAmI"]    = whoAmI;    // 0x71 MPU-9250, 0x70/0x68 clones, 0xEA ICM-20948, 0 = no reply
  top["magWhoAmI"] = magWhoAmI; // 0x48 AK8963, AK09916 WIA1 (also 0x48), 0 = not found
  top["magPath"]   = magPath;   // MPU mag access: 0=none, 1=I2C bypass, 2=I2C master
  top["magType"]   = magType;   // 0=none, 1=AK8963, 2=HMC5883L, 3=QMC5883L, 4=QMC5883P, 5=AK09916
  top["sensorKind"] = _sensorKind; // 0=none, 1=MPU-9250/ICM-20948, 2=GY-271
